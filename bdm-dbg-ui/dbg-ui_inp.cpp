...
**                  Don't touch unless you 100% sure what are you doing!
** COPYRIGHT    :   
**
****C*E******************************************************************************************/
...
void cmd_auto_complete (WCHAR *pc_cmd_line, int n_cmd_len, int *pn_cur_pos, T_UI_CMD *pt_cmd_arg)
{
    ...
    while(pt_cmd->pc_name){
        ...
        if (_wcsicmp(pt_cmd->pc_name, ca_cmd_token) == 0)  break;
        pt_cmd++;
    }
        ...
        if (ca_last_token[0] == 0){
            ...
            do{
                *ca_char = 0;
                ...
                while(pt_cmd->pc_name){
                    if (_wcsnicmp(pt_cmd->pc_name, ca_cmd_token, n_cmd_tok_len) == 0){

                        // init template for first time
                        if (*ca_char == 0 && !pt_exact_cmds){
                            *ca_char = pt_cmd->pc_name[n_cmd_tok_len];
                            n_unique = TRUE;
                        }else{
                            n_unique = FALSE;
                        }
                        ...
                        if (n_cmd_tok_len == wcslen(pt_cmd->pc_name)){
                            pt_exact_cmds = pt_cmd;
                        }

                        // is character is same
                        if (*ca_char != pt_cmd->pc_name[n_cmd_tok_len]) {
                            *ca_char = 0;
                            break;
                        }
                    }
...
int cmd_suggest (WCHAR *pc_cmd_line, int *pn_cur_pos, T_UI_CMD *pt_cmd_arg)
{
    ...
    if (pc_next_tok){
        ...
        pt_cmd = pt_cmd_arg;
        while(pt_cmd->pc_name){
            if (_wcsnicmp(pt_cmd->pc_name, ca_cmd_token, n_tok_len) == 0){
                n_cmd_hit_cnt ++;
                // Check exact match
                if (n_tok_len == wcslen(pt_cmd->pc_name)){
                    pt_hit_cmd = pt_cmd;
                }
            }
            pt_cmd++;
    ...
    if (pt_hit_cmd){
        pt_fields = pt_hit_cmd->pt_fields;    
        // Command recognized
        ...
        if (ca_last_token[0]){
            ...
            n_tok_len = (int)wcslen(ca_cmd_token);
            while(pt_cmd->pc_name){
                if (wcsncmp(pt_cmd->pc_name, ca_cmd_token, n_tok_len) == 0){
                    n_str_len = (int)wcslen(pt_cmd->pc_name);
                    if ( n_str_len > n_str_max_len) n_str_max_len = n_str_len;
                }
                pt_cmd++;
            }
            ...
            if (n_str_max_len > 0){
                ...
                while(pt_cmd->pc_name){
                    if (wcsncmp(pt_cmd->pc_name, ca_cmd_token, n_tok_len) == 0){
                        print_col_adj(pt_cmd->pc_name, n_str_max_len+2);
                    }
                    pt_cmd++;
    ...
    switch (n_rc){
        ...
        case GL_RC_RETURN:
            ...
            *pn_prompt_restore = TRUE;
            break;
        case GL_RC_TAB:
            cmd_auto_complete(&pt_info->ca_cmd_line[0], wcs_sizeof(pt_info->ca_cmd_line)-1, &n_cur_pos, pt_cmd);
        ...
        case GL_RC_TAB_TAB:
            wprintf(L"\n");
            cmd_suggest(&pt_info->ca_cmd_line[0], &n_cur_pos, pt_cmd);
            *pn_prompt_restore = TRUE;
            n_flag = 0;
            break;
    } // End of switch
...
void show_options_help(){
    ...
    wprintf(L" This tool sends IO control commands to the dedicated        \n");
    wprintf(L" (named) IO pipe.                                            \n"); 
    wprintf(L"                                                             \n");
    wprintf(L" Input arguments format:                                     \n");
    wprintf(L"    to be defined                                            \n");
    wprintf(L" Options: -v --verbose                                       \n");
    wprintf(L"   -v      TBD                                               \n");                 
    wprintf(L"                                                             \n");
    wprintf(L" Actions:  -c --command;                                     \n");
    wprintf(L"                                                             \n");
    wprintf(L" If action not specified you'll be kindly asked to enter     \n");
    wprintf(L" BTCAR DBG IO commands from console.                         \n");
    wprintf(L"                                                             \n");
