...
**                                        
** PROJECT      :   BDM debug command user interface
**                                                                
** SRC-VERSION  :   
**              
** DATE         :   
...
**                  BDM IO commands are defines in BDM-DBG-CMD-LIB project.
**                  
** COPYRIGHT    :   
**
****C*E******************************************************************************************/
...
WCHAR *raw8_to_str(T_RAW_BUF *pt_raw8)
{
    int n = pt_raw8->b_len;
    BYTE *pb_buff = pt_raw8->pb_buff;

    WCHAR *pc_str = &gc_raw_buff_str[0];
    size_t n_max_str_len = sizeof(gc_raw_buff_str) >> 1;

    while(n--) {
        int chrs = swprintf(pc_str, n_max_str_len, L"%02X ", *pb_buff);
        n_max_str_len -= chrs;
        pb_buff ++;
    }
    return gc_raw_buff_str;
}

WCHAR *raw16_to_str(T_RAW_BUF *pt_raw16)
{
    int n = pt_raw16->b_len;
    WORD *pw_buff = (WORD*)pt_raw16->pb_buff;

    WCHAR *pc_str = &gc_raw_buff_str[0];
    size_t n_max_str_len = sizeof(gc_raw_buff_str) >> 1;

    while(n--) {
        int chrs = swprintf(pc_str, n_max_str_len, L"%04X ", *pw_buff);
        n_max_str_len -= chrs;
        pw_buff ++;
    }
    return gc_raw_buff_str;
...
void ui_cmd_field_dump (T_UI_CMD_FIELD* pt_cmd_fields)
{
	...
	{
		if (pt_field->e_type == CFT_RAW8)
		{
			wprintf(L"        %-10s %d %d %s\n",
				pt_field->pc_name,
				pt_field->e_type,
				pt_field->n_max_len,
				raw8_to_str(pt_field->pt_raw_buff));
		}
        else if (pt_field->e_type == CFT_RAW16) 
        {
			wprintf(L"        %-10s %d %d %s\n",
				pt_field->pc_name,
				pt_field->e_type,
				pt_field->n_max_len,
				raw16_to_str(pt_field->pt_raw_buff));
        }
		else if (pt_field->e_type == CFT_TXT)
		{
			wprintf(L"        %-10s %d %d %s\n",
				pt_field->pc_name,
				pt_field->e_type,
...
T_UI_CMD_FIELD* io_pipe_rx_proc_ui_cmd_cdef (BYTE *pb_msg_buff_inp)
{
	...
	while (pb_msg_buff = get_tlv_tl(pb_msg_buff, &t_tlv))
	{
		...
		case UI_IO_TLV_TYPE_FLD_LEN:
			pb_msg_buff = get_tlv_v_dword(pb_msg_buff, &t_tlv);
			pt_field->n_max_len = t_tlv.val_dword;
			break;
		...
		case UI_IO_TLV_TYPE_FLD_VAL:
			...
			else if (pt_field->e_type == CFT_RAW8 ||
                     pt_field->e_type == CFT_RAW16)
			{
				pb_msg_buff = get_tlv_v_raw_n(pb_msg_buff, &t_tlv, pt_field->n_max_len);
                pt_field->pt_raw_buff = t_tlv.val_raw;
				pt_field->pt_raw_buff->pb_buff = t_tlv.val_raw->pb_buff;
                pt_field->pt_raw_buff->b_len = 0;
			}
			else if (pt_field->e_type == CFT_TXT)
			{
				pb_msg_buff = get_tlv_v_str_n(pb_msg_buff, &t_tlv, pt_field->n_max_len);
...
int io_pipe_check (int *pn_prompt_restore)
{
    ...
    if (gt_flags.io_conn == FL_RISE)
    {
        ...
        {
            n_gle = GetLastError();
            wprintf(L"Can't read from IO pipe. Error %s %d @ %d", gpc_pipe_name, n_gle, __LINE__);
            gt_flags.io_conn = FL_FALL;
        }
    }
...
int start_io(){
    ...
    { // Start IO process and pass outgoing pipe as STDIN
        ...
        wsprintf(ca_cmd_line, L"XXX-dbg-io-proc.exe %s", gpc_pipe_name);
